...
#vexpr = Expression(("0.0","0.0","0.0"))
v.interpolate(vexpr)
uinit = Function(Vu).interpolate(uexpr)
uadv = Function(state.V[0]).project(uexpr)
ubar = Function(state.V[0]).project(uadv - v)
...
timestepper = SSPRK3(D0, equation)

while t < tmax:
    t += dt

    deltax.project(dt*v)
    x0.project(x+Constant(0)*deltax)
    x2.project(x+deltax)
    x1.project(x+0.5*deltax)

    outfile.write(D0, ubar, uadv)
    coords_ll = get_latlon_coords(base_mesh)
    mesh_ll.coordinates.dat.data[:] = coords_ll.dat.data[:]
    outfilell.write(D_ll, ub_ll, u_ll)
    timestepper.apply(D0, D0, [mesh0, mesh1, mesh2], dt, deltax, vexpr, v, uadv, uexpr)
