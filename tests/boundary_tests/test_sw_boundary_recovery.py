...
def setup_sw_recovery(dirname):
    ...
    mesh = RectangleMesh(ncolumnsx, ncolumnsy, L, W, quadrilateral=True)
    ...
    np.random.seed(0)
    expr = np.random.randn() + np.random.randn()*x + np.random.randn()*y + np.random.randn()*x*y
...
def run_sw_recovery(dirname):

    (rho_diff, v_diff) = setup_sw_recovery(dirname)
    return (rho_diff, v_diff)


def test_sw_boundary_recovery(tmpdir):

    dirname = str(tmpdir)
    rho_diff, v_diff = run_sw_recovery(dirname)
