...
def setup_3d_recovery(dirname):
    ...
    m = PeriodicRectangleMesh(ncolumnsx, ncolumnsy, L, W, direction='x', quadrilateral=True)
    ...
    np.random.seed(0)
    expr = np.random.randn() + np.random.randn()*y + np.random.randn()*z + np.random.randn()*y*z
