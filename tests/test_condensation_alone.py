...
# This set up tests the condensation procedure.
# A blob of over-saturated air is initialised, with no cloud.
# The excess water vapour should condense, releasing latent heat.
# There is no advection.

# This tests that the condensation happens, consevering mass of water,
# releasing heat, and not forming negative water concentrations.
...
def run(setup):
    ...
    # set up water_v, which is sure to be over-saturated
    xc = 0.5 * Ld
    zc = 0.35 * Ld
    rc = 0.25 * Ld
...
def test_condensation(tmpdir, moist_setup):

    setup = moist_setup(tmpdir, "normal")
    water_change, cloud, theta_change, negative_water_v, negative_cloud = run(setup)
    # check that the total amount of water hasn't changed
    assert water_change < 1e-12
    # check that a cloud has indeed formed
    assert cloud > 1e-12
    # check that heat has been released
