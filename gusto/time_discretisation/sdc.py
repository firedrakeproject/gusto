...
__all__ = ["SDC", "IDC", "RIDC", "RIDC_Reduced", "Parallel_RIDC", "Parallel_SDC"]
...
class IDC(object, metaclass=ABCMeta):
    ...
    def __init__(self, base_scheme, domain, M, K, field_name=None,
        ...
        print("dt", float(self.dt))
        self.nodes = np.arange(0, (M+1)*float(self.dt), float(self.dt))
        integration_matrix = self.lagrange_integration_matrix(self.M+1)

        # Rescale nodes from [0, 1] to [0, dt]
        self.Q = float(self.dt_coarse) * integration_matrix
...
class RIDC(object, metaclass=ABCMeta):
    ...
    def compute_quad_final(self, Q, fUnodes, m):
        ...
        quad.assign(0.)
        for k in range(0, self.K+1):
            quad += float(Q[-1, k])*fUnodes[m - self.K + k]
        return quad
...
class RIDC_Reduced(object, metaclass=ABCMeta):
    """Class for Integral Deferred Correction schemes."""
...
class Parallel_RIDC(object, metaclass=ABCMeta):
    """Class for Integral Deferred Correction schemes."""
    ...
    def apply(self, x_out, x_in):
        ...
        #logger.info(f'Communicator: {self.kval:.2e}')
        #breakpoint()
        self.Un.assign(x_in)
        self.Unodes[0].assign(self.Un)
        # Loop through quadrature nodes and solve
        ...
        # On first communicator
        if (self.comm.ensemble_comm.rank == 0):
            logger.info(f'Starting base timestepper: {self.kval:.2e}')
            # base timestepper

            for m in range(self.M):
                self.base.dt = float(self.dt)
                logger.info(f'Base stepper: {self.kval:.2e}')
                self.base.apply(self.Unodes[m+1], self.Unodes[m])
                logger.info(f'Base stepper done: {self.kval:.2e}')
                for evaluate in self.evaluate_source:
                ...
                #     self.comm.send(self.node_count, dest=int(i), tag=11)  # Send data to all other processes
                self.comm.send(self.Unodes[m+1], dest=self.kval+1, tag=100+m+1)
                logger.info(f'Sent data to process {self.kval+1:.2e} from process {self.kval:.2e}')

        else:
            for m in range(1, self.kval + 1):
                self.comm.recv(self.Unodes[m], source=self.kval-1, tag=100+m)
                logger.info(f'Recieved data to process {self.kval:.2e} from process {self.kval-1:.2e}')
                self.Uin.assign(self.Unodes[m])
                for evaluate in self.evaluate_source:
                    evaluate(self.Uin, self.base.dt, x_out=self.source_in)
                self.solver_rhs.solve()
                self.fUnodes[m].assign(self.Urhs)
            # if k == 1:
            #     for m in range(0, self.kval*(self.kval+1)/2):
            #         self.Unodes[m] = self.comm.recv(source=0, tag=123)
            # else:
            #     for m in range(0, self.kval*(self.kval+1)/2):
            #         self.Unodes1[m] = self.comm.recv(source=self.kval-1, tag=11)
            #         self.Unodes[m].assign(self.Unodes1[m])
            for m in range(0, self.kval):
                # if (m >= self.kval*(self.kval+1)/2):
                #     self.Unodes1[m] = self.comm.recv(source=self.kval-1, tag=11)
                #     self.Unodes[m].assign(self.Unodes1[m])
                # Get f(u)
                # if (m > self.kval+2):
        ...
        if (self.kval == self.K):
            logger.info(f'Broadcasting for: {self.kval:.2e}')
...
class Parallel_SDC(SDC):
    ...
    def apply(self, x_out, x_in):
        ...
        while k < self.maxk:
            ...
            self.U_SDC.assign(self.Unodes[self.comm.ensemble_comm.rank+1])
            ...
            #             + sum(j=1,M)  Q_delta_exp[m,j]*(S(y_(m-1)^(k+1)) - S(y_(m-1)^k))
            self.solver.solve()
            self.Unodes1[self.comm.ensemble_comm.rank+1].assign(self.U_SDC)
