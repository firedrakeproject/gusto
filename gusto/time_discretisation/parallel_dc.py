...
from gusto.time_discretisation.time_discretisation import wrapper_apply, wrapper_apply2
from qmat import genQDeltaCoeffs
from gusto.time_discretisation.deferred_correction import SDC, RIDC, RIDC_new

__all__ = ["Parallel_RIDC", "Parallel_SDC", "Parallel_RIDC_new", "Parallel_RIDC_new2", "Parallel_RIDC_flush"]
...
class Parallel_RIDC_new(RIDC_new):
    ...
    def __init__(self, base_scheme, domain, M, K, J, flush_freq = None, field_name=None,
        ...
        self.TAG_EXCHANGE_SOURCE = 100 # Tag for sending nodal source fields (Firedrake Functions)
    ...
    def apply(self, x_out, x_in):
        ...
        for j in range(1, self.J + 1):
            ...
            time = (j-1) * float(self.dt_coarse)
            print("RIDC parallel step ", j, " on rank ", self.kval, "time:", time)
            self.Unodes[0].assign(self.Un)
            # Loop through quadrature nodes and solve
            self.Unodes1[0].assign(self.Unodes[0])
            for evaluate in self.evaluate_source:
            ...
            if (self.comm.ensemble_comm.rank == 0):
                ...
                for m in range(self.M):
                    ...
                    # Send base guess to k+1 correction
                    print("Rank", self.kval, "sending node ", m+1, " to rank", self.kval+1 ,"at step ", j)
                    self.comm.send(self.Unodes[m+1], dest=self.kval+1, tag=self.TAG_EXCHANGE_FIELD+j)
                    self.comm.send(self.source_Uk[m+1], dest=self.kval+1, tag=self.TAG_EXCHANGE_SOURCE+j)
            else:
                for m in range(1, self.kval + 1):
                    # Recieve and evaluate the stencil of guesses we need to correct
                    print("Rank", self.kval, "receiving node ", m, " from rank", self.kval-1 ,"at step ", j)
                    self.comm.recv(self.Unodes[m], source=self.kval-1, tag=self.TAG_EXCHANGE_FIELD+j)
                    self.comm.recv(self.source_Uk[m], source=self.kval-1, tag=self.TAG_EXCHANGE_SOURCE+j)
                    self.Uin.assign(self.Unodes[m])
                    for evaluate in self.evaluate_source:
                        evaluate(self.Uin, self.base.dt, x_out=self.source_in)
                    self.solver_rhs.solve()
                ...
                for m in range(0, self.kval):
                    ...
                    if self.kval < self.K:
                        print("Rank", self.kval, "sending node ", m+1, " to rank", self.kval+1 ,"at step ", j)
                        self.comm.send(self.Unodes1[m+1], dest=self.kval+1, tag=self.TAG_EXCHANGE_FIELD+j)
                        self.comm.send(self.source_Ukp1[m+1], dest=self.kval+1, tag=self.TAG_EXCHANGE_SOURCE+j)

                for m in range(self.kval, self.M):
                    # Recieve the guess we need to correct and evaluate the rhs
                    print("Rank", self.kval, "receiving node ", m+1, " from rank", self.kval-1 ,"at step ", j)
                    self.comm.recv(self.Unodes[m+1], source=self.kval-1, tag=self.TAG_EXCHANGE_FIELD+j)
                    self.comm.recv(self.source_Uk[m+1], source=self.kval-1, tag=self.TAG_EXCHANGE_SOURCE+j)
                    self.Uin.assign(self.Unodes[m+1])
                    for evaluate in self.evaluate_source:
                        evaluate(self.Uin, self.base.dt, x_out=self.source_in)
                    self.solver_rhs.solve()
            ...
            # print("Rank", self.kval, "finished interval ", j)
            # if (self.kval == self.K):
            #     x_out.assign(self.Unodes[-1])
            #     for i in range(self.K):
            #         self.comm.send(x_out, dest=i, tag=self.TAG_FLUSH_PIPE)
            # else:
            #     self.comm.recv(x_out, source=self.K, tag=self.TAG_FLUSH_PIPE)
            if self.flush_freq > 0 and j % self.flush_freq == 0:
        ...
        #max_diff = x_diff.dat.data_ro.max()

class Parallel_RIDC_flush(RIDC):
    """Class for Parallel Revisionist Integral Deferred Correction schemes."""
    ...
    def __init__(self, base_scheme, domain, M, K, J, flush_freq = None, field_name=None,
        ...
        super(Parallel_RIDC_flush, self).__init__(base_scheme, domain, M, K, field_name,
                                            linear_solver_parameters, nonlinear_solver_parameters,
                                            limiter, options, reduced=True)
        self.comm = communicator
    ...
    def setup(self, equation, apply_bcs=True, *active_labels):
        ...
        Args:
            ...
            *active_labels (:class:`Label`): labels indicating which terms of
                the equation to include.
        """
        super(Parallel_RIDC_flush, self).setup(equation, apply_bcs, *active_labels)
...
class Parallel_RIDC_new2(RIDC_new):
    """Class for Parallel Revisionist Integral Deferred Correction schemes."""

    def __init__(self, base_scheme, domain, M, K, J, field_name=None,
                 linear_solver_parameters=None, nonlinear_solver_parameters=None,
                 limiter=None, options=None, communicator=None):
        """
        Initialise RIDC object
        ...
        super(Parallel_RIDC_new2, self).__init__(base_scheme, domain, M, K, J, field_name,
                                            linear_solver_parameters, nonlinear_solver_parameters,
                                            limiter, options, reduced=True)
        self.comm = communicator
    ...
    def setup(self, equation, apply_bcs=True, *active_labels):
        ...
        Args:
            ...
            *active_labels (:class:`Label`): labels indicating which terms of
                the equation to include.
        """
        super(Parallel_RIDC_new2, self).setup(equation, apply_bcs, *active_labels)
