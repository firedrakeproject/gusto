...
__all__ = ["BE_SDC", "FE_SDC", "IMEX_SDC","IMEX_SDC_QD","IM_SDC"]
...
class SDC(object, metaclass=ABCMeta):
    ...
    def __init__(self, base_scheme, domain, M, maxk, node_type, node_dist, qdelta_imp, qdelta_exp,
        ...
        if formulation == "zero-to-node":
            self.compute_quad = self.compute_quad_zero2n
        elif formulation == "node-to-node":
            self.compute_quad = self.compute_quad_n2n
        else:
    ...
    def solver_SDC(self):
        ...
        prob_SDC = NonlinearVariationalProblem(self.res_n2n, self.U_SDC, bcs=self.bcs)
...
class BE_SDC(SDC):
    ...
    def res_n2n(self):
        ...
        # # y_m^(k+1) + dt*F(y_(m)^(k+1))
        # a = F.label_map(lambda t: all_terms,
        #                 replace_subject(self.U_SDC, old_idx=self.idx))

        # # y_(m-1)^(k+1)
        # F_imp = F.label_map(all_terms, replace_subject(self.Uk, old_idx=self.idx))
        ...
        residual = mass_form.label_map(all_terms,
                                       map_if_true=replace_subject(self.U_SDC, old_idx=self.idx))
        residual -= mass_form.label_map(all_terms,
                                        map_if_true=replace_subject(self.Uk, old_idx=self.idx))
        ...
        r_k = self.residual.label_map(
                lambda t: t.has_label(time_derivative),
                map_if_true=drop,
                map_if_false=replace_subject(self.U01, old_idx=self.idx))
        r_k = r_k.label_map(
            all_terms,
            lambda t: self.dt*t)
