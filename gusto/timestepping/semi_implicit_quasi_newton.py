...
from firedrake import (
    ...
    TestFunction, TrialFunction, inner, dx
)
...
from gusto.equations import ShallowWaterEquations
...
class SemiImplicitQuasiNewton(BaseTimestepper):
    ...
    def timestep(self):
        ...
        for outer in range(self.num_outer):
            ...
            for i_inner in range(self.num_inner):

                # Implicit forcing ---------------------------------------------
                with timed_stage("Apply forcing terms"):
                    logger.info(f'Semi-implicit Quasi Newton: Implicit forcing {(outer, i_inner)}')
                    self.forcing.apply(xp, xnp1, xrhs, "implicit")
                    xrhs += xrhs_phys

                    if (i_inner > 0 and self.accelerator):
                ...
                if abs(self.physics_beta) > 0.0:
                    for _, scheme in self.inner_physics_schemes:
                        logger.info(f'Semi-implicit Quasi Newton: Implicit physics {(outer, i_inner)}')
                ...
                with timed_stage("Implicit solve"):
                    logger.info(f'Semi-implicit Quasi Newton: Mixed solve {(outer, i_inner)}')
...
class MeshMovement(SemiImplicitQuasiNewton):
    ...
    def __init__(self, equation_set, io, transport_schemes,
        ...
        super().__init__(equation_set=equation_set,
                         ...
                         final_physics_schemes=final_physics_schemes,
                         **kwargs)
