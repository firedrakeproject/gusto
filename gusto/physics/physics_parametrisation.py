...
class SourceSink(PhysicsParametrisation):
    ...
    def __init__(self, equation, variable_name, rate_expression,
        ...
        if hasattr(equation, "field_names"):
            ...
            self.interpolated_source = self.source.subfunction(self.V_idx)
            self.eqn_source = split(self.source)[self.V_idx]
        else:
            V = equation.function_space
            test = equation.test
            self.source = Function(V)
            self.eqn_source = self.source
            self.interpolated_source = self.source

        equation.residual += source_label(self.label(subject(test * self.eqn_source * dx, equation.X),
                                        self.evaluate))
        ...
        if self.method == 'interpolate':
            self.source_interpolator = Interpolator(expression, self.interpolated_source)
        else:
            self.source_projector = Projector(expression, self.interpolated_source)
    ...
    def evaluate(self, x_in, dt, x_out=None):
        ...
        Args:
            ...
            x_out: (:class: 'Function'): the output field for the source term
        """
        if self.time_varying:
