...
class SourceSink(PhysicsParametrisation):
    ...
    def __init__(self, equation, variable_name, rate_expression,
        ...
        if self.method == 'interpolate':
            self.source_interpolator = Interpolator(expression, self.source)
        else:
            self.source_projector = Projector(expression, self.source)
    ...
    def evaluate(self, x_out, x_in, dt):
        ...
        if self.time_varying:
            ...
            pass
        if hasattr(self.equation, "field_names"):
            x_out.subfuntion(self.V_idx).assign(self.source)
        else:
