...
class InstantRain(PhysicsParametrisation):
    ...
    def __init__(self, equation, saturation_curve,
        ...
        equation.residual += source(self.label(subject(test_v * self.source * dx,
                                                equation.X),
                                        self.evaluate))
        ...
        if rain_name is not None:
            ...
            equation.residual -= source(self.label(subject(test_r * self.source * dx,
                                                    equation.X),
                                            self.evaluate))

        # if feeding back on the height adjust the height equation
        if convective_feedback:
            equation.residual += source(self.label(subject(test_D * beta1 * self.source * dx,
                                                    equation.X),
                                            self.evaluate))
