...
from firedrake import (
    ...
    assemble, split, sqrt, dot, Constant, inner
)
...
__all__ = ["InstantRain", "SWSaturationAdjustment", "SWDrag"]
...
class SWDrag(PhysicsParametrisation):
    """
    The process of adding a darg term that depends on u to the u equation.
    """
    ...
    def __init__(self, equation, parameters=None):
        ...
        W = equation.function_space
        Vu = W.sub(self.u_idx)
        test_u = equation.tests[self.u_idx]

        # Define the source
        self.u = Function(Vu)
        self.source = Function(W)
        self.source_expr = split(self.source)[self.u_idx]
        self.source_int = self.source.subfunctions[self.u_idx]

        # Interpolator computes the expression
        u_mag = sqrt(dot(self.u, self.u))
        drag_expr = Constant(100) * equation.parameters.H * u_mag * self.u
        self.source_interpolate = interpolate(drag_expr,
                                              self.source_int.function_space())
        
        # Subtract from the residual
        equation.residual -= source_label(self.label(subject(inner(test_u, self.source_expr) * dx,
                                                                   self.source),
                                                                   self.evaluate))
    ...
    def evaluate(self, x_in, dt, x_out=None):
        ...
        Computes the drag contribution at each timestep.
