...
class VP20condensation(PhysicsParametrisation):
    """"
    Represents the term 'C' from Vallis and Penn 2020 moist shallow water model
    """
    ...
    def __init__(self, equation, saturation_curve,
        ...
        Args:
            ...
            saturation_curve (:class:`ufl.Expr` or func): the curve which
                dictates when phase changes occur. In a saturated atmosphere,
                vapour above the saturation curve instantly rains out and is
                removed from the atmosphere. The saturation curve is either
                prescribed or dependent on a prognostic field.
        """

        label_name = 'VP20condensation'

        super().__init__(equation, label_name, parameters=parameters)

        self.VD_idx = equation.field_names.index('D')
        self.Vv_idx = equation.field_names.index(vapour_name)
        V_idxs = [self.Vv_idx, self.VD_idx]
        ...
        if tau is not None:
            ...
            self.set_tau_to_dt = True
            self.tau = Function(R)
            logger.info("Timescale for moisture removal from vapour to rain-out has been set to dt. If this is not the intention then provide a tau parameter as an argument to VP20condensation.")
        ...
        # Factors for multiplying source for different variables
        # the order matches the order in V_idx (vapour, depth)
        factors = [-1, -1*self.gamma]
        ...
        for test, source_val in zip(tests, self.source_expr):
            equation.resiudal += source_label(
                self.label(subject(test * source_val * dx, self.source), self.evaluate)
            )
    ...
    def evaluate(self, x_in, dt, x_out=None):
        ...
        Computes the physics contributions to water vapour at each timestep.
        ...
        for interpolator, src in zip(self.source_interpolate, self.source_int):
            src.assign(assemble(interpolator))
        # If a source is provided, assign the source term to it
        if x_out is not None:
            x_out.assign(self.source)