...
def equiangular_weights(x0, x1, num_points):
    """
    Given two points x0 and x1, returns the weights w for
        x0 + w * (x1 - x0)
    describing n points with equiangular separation.

    :arg x0: Coordinates of first point
    :arg x1: Coordinates of second point
    :arg num_points: the total number of points, including x0 and x1
    """

    mod_x0 = np.linalg.norm(x0)
    mod_x1 = np.linalg.norm(x1)

    # Theta is the angle between x0 and x1, which we find from dot product
    theta = np.arccos(np.dot(x0, x1) / (mod_x0 * mod_x1))

    # divide theta up evenly
    dtheta = np.linspace(0, theta, num=num_points)

    # Now we want to turn this into weights so we can get spatial positions
    # solve:
    # dot(x0, w*x1 +(1-w)*x2) = mod_x0*mod(w*x1+(1-w)*x0)*cos(dtheta)
    # Rearranging gives quadratic form for w:
    # A w**2 + B w + C = 0
    A = (mod_x0**2*(np.cos(dtheta))**2
         * (mod_x0**2 + mod_x1**2 - 2*np.dot(x0,x1))
         - (np.dot(x0,x1) - mod_x0**2)**2)
    B = (2*mod_x0**2*(np.cos(dtheta))**2
         * (np.dot(x0,x1) - mod_x0**2)
         - 2*mod_x0**2*(np.dot(x0,x1) - mod_x0**2))
    C = mod_x0**4*((np.cos(dtheta))**2-1)

    edge_weights = (-B + np.sqrt(B**2 - 4*A*C)) / (2*A)
...
def GeneralIcosahedralSphereMesh(radius, num_cells_per_edge_of_panel=1,
    ...
    :kwarg degree: polynomial degree of coordinate space (defaults
        to 1: flat triangles)
    :kwarg method: string giving method for refinement. Either equidistant
        or equiangular.
    ...
    if method not in ['equidistant', 'equiangular']:
        raise ValueError('Method %s is not valid for Icosahedral refinement' % method)
    ...
    if method == 'equidistant':
        ...
        # We take the vectors corresponding to the first two icosahedron vertices
        x0 = base_vertices[0]
        x1 = base_vertices[1]

        edge_weights = equiangular_weights(x0, x1, big_N+1)
        ...
        for i, this_panel_edges in enumerate(panels_to_edges):
            ...
            for j in range(big_N - 2):
                ...
                if method == 'equidistant':
                    ...
                    face_row_weights = equiangular_weights(x0, x1, big_N-j)
