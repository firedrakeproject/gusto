...
from firedrake import (Function, TestFunction, TrialFunction, lhs, rhs, inner,
                       ...
                       FunctionSpace, Projector, DirichletBC)
import ufl

__all__ = ["ConservativeProjector", "ContinuousConservativeProjector"]
...
class ConservativeProjector(object):
    ...
    def __init__(self, rho_source, rho_target, m_source, m_target):
        ...
        eqn = (rho_source*inner(test, m_source)*dx
               - rho_target*inner(test, m_trial)*dx)
...
class ContinuousConservativeProjector(object):
    """
    Performs a conservative projection for a continuous field.

    This object is designed for projecting fields such as mixing ratios of
    tracer species from one function space to a continuous space, weighted by a
    density such that mass is conserved by the projection, and a constant mixing
    ratio field.
    """
    ...
    def __init__(self, rho_source, rho_target, m_source, m_target):
        ...
        # Create tracer mass field
        # tracer_mass = Function(DG0)
        # self.mass_projector = Projector(rho_source*m_source, tracer_mass)

        test = TestFunction(V)
        m_trial = TrialFunction(V)
        eqn = inner(test, m_source)*rho_source*dx - rho_target*inner(test, m_trial)*dx
        bcs = [DirichletBC(V, m_source, 'bottom'), DirichletBC(V, m_source, 'top')]
        problem = LinearVariationalProblem(lhs(eqn), rhs(eqn), self.m_target, bcs=bcs)
        self.solver = LinearVariationalSolver(problem)

    def project(self):
        """Apply the projection."""
        # self.mass_projector.project()
        self.solver.solve()
        return self.m_target
