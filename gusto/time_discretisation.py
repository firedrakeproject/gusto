...
class IMEX_SDC2(SDC):
    ...
    def setup(self, equation, uadv=None):
        ...
        self.IMEX.setup(equation)
        self.residual = self.IMEX.residual
        self.field_name=self.IMEX.field_name
        self.solver_parameters=self.IMEX.solver_parameters
        ...
        F_imp = F.label_map(lambda t: any(t.has_label(implicit,time_derivative)),
                            map_if_true=replace_subject(self.U_SDC),
                            map_if_false=drop)

        F_exp = F.label_map(lambda t: any(t.has_label(explicit,time_derivative)),
                            map_if_true=replace_subject(self.Un),
                            map_if_false=drop)

        F_exp = F_exp.label_map(lambda t: t.has_label(time_derivative),
                                lambda t: -1*t)

        F0 = F.label_map(lambda t: any(t.has_label(time_derivative, implicit)),
                         drop,
                         replace_subject(self.U0))
        # F0 = F0.label_map(all_terms,
        #                   lambda t: -1*t)
        
        F01 = F.label_map(lambda t: any(t.has_label(time_derivative, explicit)),
                         drop,
                         replace_subject(self.U01))
        F01 = F01.label_map(all_terms,
                          lambda t: -1*t)
        
        # F_imp = self.residual.label_map(lambda t: any(t.has_label(time_derivative,implicit)),
        #                      replace_subject(self.U_SDC),
        #                      drop)
        # F_imp = F_imp.label_map(lambda t: t.has_label(time_derivative),
        #                         map_if_false= lambda t: self.dt*t)
        # F_exp = self.residual.label_map(lambda t: any(t.has_label(time_derivative,explicit)),
        #                      replace_subject(self.Un),
        #                      drop)
        # F_exp = F_exp.label_map(lambda t: t.has_label(time_derivative),
        #                         map_if_false= lambda t: -self.dt*t)

        # F01 = self.residual.label_map(lambda t: t.has_label(time_derivative),
        #                                 drop,
        #                                 replace_subject(self.U01))
        # F01 = F01.label_map(lambda t: t.has_label(implicit),
        #                     map_if_true=lambda t: -self.dt*t,
        #                     map_if_false=drop)

        # F0 = self.residual.label_map(lambda t: t.has_label(time_derivative),
        #                                 drop,
        #                                 replace_subject(self.U0))
        # F0 = F0.label_map(lambda t: t.has_label(explicit),
        #                   map_if_true=lambda t: self.dt*t,
        #                   map_if_false=drop  )
        Q = self.residual.label_map(lambda t: t.has_label(time_derivative),
                                    replace_subject(self.Q_),
    ...
    def lhs_quad(self):
        l = self.residual.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_true=replace_subject(self.Urhs),
            map_if_false=drop)
        return l.form
    ...
    def rhs_quad(self):
        r = self.residual.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_true=drop,
            map_if_false=replace_subject(self.Uin))
        return r.form

    @cached_property
    def solver_quad(self):
        # setup solver using lhs and rhs defined in derived class
        problem = NonlinearVariationalProblem(self.lhs_quad - self.rhs_quad, self.Urhs, bcs=self.bcs)
        solver_name = self.field_name+self.__class__.__name__
        return NonlinearVariationalSolver(problem, solver_parameters=self.solver_parameters, options_prefix=solver_name)

    @property
    def lhs_sdc(self):
        l1 = self.residual.label_map(
            lambda t: any(t.has_label(implicit, time_derivative)),
            map_if_true=replace_subject(self.U_SDC),
            map_if_false=drop)
        l1 = l1.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_false=lambda t: self.dt*t)
        l2 = self.residual.label_map(
            lambda t: t.has_label(implicit),
            map_if_true=replace_subject(self.U01),
            map_if_false=drop)
        l2 = l1.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_false=lambda t: self.dt*t)
    ...
    def rhs_sdc(self):
        ...
        r1 = r1.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_false=lambda t: -self.dt*t)
        r2 = self.residual.label_map(
            lambda t: t.has_label(explicit),
            map_if_true=replace_subject(self.U0),
            map_if_false=drop)
        r2 = r2.label_map(
            lambda t: t.has_label(time_derivative),
            map_if_false=lambda t: -self.dt*t)
    ...
    def Q_sdc(self):
        Q = self.residual.label_map(lambda t: t.has_label(time_derivative),
                                        map_if_true=replace_subject(self.Q_),
                                        map_if_false=drop)
        return Q.form
    ...
    def solver(self):
        # setup solver using lhs and rhs defined in derived class
        problem = NonlinearVariationalProblem(self.lhs_sdc - self.rhs_sdc + self.Q_sdc, self.U_SDC, bcs=self.bcs)
        solver_name = self.field_name+self.__class__.__name__
