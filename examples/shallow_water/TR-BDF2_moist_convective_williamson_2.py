...
from firedrake import SpatialCoordinate, sin, cos, exp, Function, sqrt
from gusto import (
    ...
    GeneralCubedSphereMesh, xyz_vector_from_lonlatr, 
)
...
def moist_convect_williamson_2(
        ...
        case=moist_convect_williamson_2_defaults['case']
):
    ...
    print(f'Optimal cores is {eqns.X.function_space().dim() / 50000}')
    # IO
    ...
    if case=='A':
        stepper = TRBDF2QuasiNewton(eqns, io, transported_fields, 
                                    spatial_methods=transport_methods,
                                    gamma=gamma,
                                    tr_solver=tr_linear_solver,
                                    bdf_solver=bdf_linear_solver,
        ...
        stepper = TRBDF2QuasiNewton(eqns, io, transported_fields, 
                                    ...
                                    tr_solver=tr_linear_solver,
                                    bdf_solver=bdf_linear_solver,
                                    scaled_final_physics_schemes=physics_schemes,
                                    middle_physics_schemes=mid_physics
                                    )
    elif case == 'B.1':
        stepper = TRBDF2QuasiNewton(eqns, io, transported_fields, 
                                    spatial_methods=transport_methods,
                                    ...
                                    tr_solver=tr_linear_solver,
                                    bdf_solver=bdf_linear_solver,
                                    middle_physics_schemes=physics_schemes
                                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--case',
        help="Which case to run: A (physics at end of step), B (physics at mid and end of step), B.1 (physics at mid-step only)",
        type=str,
        default=moist_convect_williamson_2_defaults['case']
    )
