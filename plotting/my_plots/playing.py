...
# if folder == 'vp20_moist_jupiter':
#     file = 'single-step_trap-qg_tophat_radt10cD1em3gamma390000q05em1xi1em1_Bu1b1p5Rop2_l10dt250df10'
#     rain_extra, _ = field_domain_integral(file, 'rain', rlim, L)
#     rain_diff_extra = xr.concat([rain_extra.isel(time=0), rain_extra.diff(dim='time')], dim='time')
#     rain_diff_extra.plot(ax=axs[1,0], color='k', linestyle='--', label='390000, rad=10')
# else:
#     file = 'single-step_trap_radt10beta390000q01em2xi1em1_Bu1b1p5Rop2_l10dt250df10'
#     cloud_extra, _ = field_domain_integral(file, 'cloud_water', rlim, L)
#     cloud_extra.plot(ax=axs[1,0], color='k', linestyle='--', label='rad=10')
# D_L2_extra, _ = field_domain_L2_error_integral(file, 'D', rlim, L)
# D_L2_extra.plot(ax=axs[1,1], color='k', linestyle='--', label='390000, rad=10')

if folder == 'vp20_moist_jupiter':
    file = 'single-step_trap-qg_tophat_cD1em3gamma390000q05em1xi1em1_Bu1b1p5Rop2_l50dt250df10'
    rain_extra, _ = field_domain_integral(file, 'rain', rlim, L)
    rain_diff_extra = xr.concat([rain_extra.isel(time=0), rain_extra.diff(dim='time')], dim='time')
    rain_diff_extra.plot(ax=axs[1,0], color='k', linestyle='--', label='longer no rad')
else:
    file = 'single-step_trap_beta390000q01em2xi1em1_Bu1b1p5Rop2_l50dt250df10'
    cloud_extra, _ = field_domain_integral(file, 'cloud_water', rlim, L)
    cloud_extra.plot(ax=axs[1,0], color='k', linestyle='--', label='rad=10')
D_L2_extra, _ = field_domain_L2_error_integral(file, 'D', rlim, L)
D_L2_extra.plot(ax=axs[1,1], color='k', linestyle='--', label='longer no rad')
...
plt.savefig(f'{path}/integral_timeseries{extra_name}_playing.pdf')
print(f'Plot made:\n{path}/integral_timeseries{extra_name}_playing.pdf')