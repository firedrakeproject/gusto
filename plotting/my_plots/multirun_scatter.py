...
def plotting(i, relax_type, path=path):
    ...
    plot0 = axs[0,0].scatter(x=dq_mean, y=q_eddens_mean, label=i, marker=m, color=colour)
    axs[0,0].text(dq_mean+0.03e-9, q_eddens_mean, i)
    axs[0,0].set_ylabel('Eddy enstrophy')
    axs[0,0].set_xlabel('dq')
    plot1 = axs[1,0].scatter(x=dphi_mean, y=q_eddens_mean, label=i, marker=m, color=colour)
    axs[1,0].text(dphi_mean+0.15, q_eddens_mean, i)
    ...
    tracer_plot = pole_tracer_frac.plot(ax=axs[0,2], color=colour, label=f'{i} max, {lat_thresh.values:.2f}')
    tracer_grad_plot = pole_tracer_grad_frac.plot(ax=axs[0,2], linestyle='--', color=colour, label=f'{i} grad, {lat_thresh_grad.values:.2f}')
    axs[0,2].set_ylabel('Poleward fraction of tracer')
    ...
    axs[0,1].set_ylabel('Final tracer fraction')
    axs[0,1].set_xlabel('Eddy enstrophy')
    axs[0,1].text(q_eddens_mean+0.01, pole_tracer_frac[-1], f'{i}')
    ...
    fft_plot = fft_ave_cut.plot(ax=axs[1,2], color=colour, marker=m, label=i)
    ...
    plot7 = axs[2,1].scatter(x=condensing_fraction_mean, y=pole_tracer_frac[-1], s=q_eddens_mean*130, color=colour, marker=m)
    axs[2,1].text(condensing_fraction_mean+0.1, pole_tracer_frac[-1], f'{i}, {q_eddens_mean:.1f}')
    axs[2,1].set_ylabel('Final tracer fraction')
    axs[2,1].set_xlabel('Mean condensing area fraction')
    q_late_mean.plot(ax=axs[1,1], color=colour, linestyle=l, label=i)
    axs[1,1].legend()
