...
from firedrake import (
    cos, sin, SpatialCoordinate, pi, assemble, dx
)
...
def setup_mean_mixing_ratio(dirname):
    ...
    m_X = ActiveTracer(name='m_X', space=tracer_space,
    ...
    # Slotted cylinders
    m_X_0 = conditional(great_arc_angle(lamda, theta, lamda_c1, theta_c1) < 0.5,
                        conditional(abs(lamda - lamda_c1) < 1./12.,
                                    conditional(theta - theta_c1 < -5./24., 1.0, 0.0),
                                    1.0),
                        conditional(great_arc_angle(lamda, theta, lamda_c2, theta_c2) < 0.5,
...
def test_mean_mixing_ratio(tmpdir):
    ...
    stepper, dt, rho_X_0 = setup_mean_mixing_ratio(dirname)
