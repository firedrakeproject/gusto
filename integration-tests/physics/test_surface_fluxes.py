...
def run_surface_fluxes(dirname, moist, implicit_formulation):
    ...
    temperature = Function(Vt).interpolate(Constant(290.))
    ...
    if moist:
        ...
        water_v0.interpolate(0.9*mv_sat)
...
@pytest.mark.parametrize("implicit_formulation", [True, False])
def test_surface_fluxes(tmpdir, moist, implicit_formulation):
    ...
    T.project(T_expr)
    denom = norm(T)
    assert norm(T - T_true) / denom < 0.01, f'Final temperature is incorrect'
